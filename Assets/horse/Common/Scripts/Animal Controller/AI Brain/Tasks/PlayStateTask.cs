...
namespace MalbersAnimations.Controller.AI
{
    ...
    public class PlayStateTask : MTask
    {
        ...
        public bool PlayOnTarget = false;
        ...
        {
            if (PlayOnTarget && !brain.AIMovement.HasArrived) return; //Dont play if Play on target is true but we are not near the target.

            switch (affect)
            {
                case Affected.Self:
                    DoStateAction(brain.Animal); 
                    break;
                case Affected.Target:
                    if (brain.TargetAnimal) DoStateAction(brain.TargetAnimal);
                    break;
                default:
                    break;
            }
        ...
        private void DoStateAction(MAnimal CurrentAnimal)
        {
            switch (action)
            {
