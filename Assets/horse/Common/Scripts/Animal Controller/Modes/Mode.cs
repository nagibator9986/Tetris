...
namespace MalbersAnimations.Controller
{
    ...
    public class Mode
    {
        ...
        public virtual void TryActivate()
        {
            ...
            if (StateBlockMode(newAbility)) return; //Check if the States can block the mode
            ...
            if (Animal.IsPlayingMode)              // Means the there's a Mode Playing
            {
                ...
                {
                    if (!ActiveMode.HasCoolDown) return; //Means that the Animations needs to finish first if the Active Mode Has no Cool Down so skip the code
            ...
            if (InCoolDown) return;                     //Exit the Mode After the cooldown has Passed no matter if is NOT PLAYING A MODE *GOOOD CODE
        ...
        public void AnimationTagEnter(int animatorTagHash)
        {
            ...
            if (ActiveAbility != null)
            {
                ...
                //ModeActivatedTime = Time.time;                 //Save the Time the Mode was active

                //Debug.Log("ModeActivatedTimeModeActivatedTimeModeActivatedTime" + ModeActivatedTime);
