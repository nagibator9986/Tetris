       ...
       // public float OuterAngle = 15f;
        ...
        public override bool TryActivate()
        {
            ...
            if (findClimbable)
            {
                if (automatic)
                {
                    return WallClimbTag = ClimbHit[0].transform.gameObject.CompareTag(ClimbTag);
                }
                return WallClimbTag = ClimbHit[0].transform.gameObject.CompareTag(ClimbTag) && InputValue;
            }
                ...
                if (!LedgeHit)
                {
                    ...
                    if (LedgeHit)
                    {
                        ...
                        if (hit.distance > LedgeMinExit*scalefactor)
                        {
                            ...
                            animal.SetIntID(Int_ID.AllowExit);
