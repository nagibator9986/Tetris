...
namespace MalbersAnimations.Controller
{
    ...
    public class MAnimalEditor : Editor 
    {
      ...
      //  private ReorderableList Reo_List_Pivots;
        ...
        SerializedProperty
            S_StateList, S_PivotsList,/* Height,*/ S_Mode_List, Editor_Tabs1, Editor_Tabs2, /*ModeIndexSelected,*/ OnEnterExitStances, ModeShowAbilities, OnEnterExitStates, RB,Anim, //MovementDeathValue, //PivotPosDir,
            m_Vertical, m_Horizontal, m_IDFloat, m_IDInt, m_State, m_LastState, m_Mode, m_Grounded, m_Movement, m_Random, m_SpeedMultiplier, m_UpDown, /*OnMainPlayer, */currentStance, defaultStance,
        ...
        { 
            ...
            if (Index != -1)
            {
                ...
                MalbersEditor.Foldout(ModeShowAbilities, "Abilities");
        ...
        private void Draw_Header_Modes(Rect rect)
        {
            ...
            r.x += 60;
            r.width = 60;
            EditorGUI.LabelField(r, new GUIContent("Mode ID", "Modes are the Animations that can be played on top of the States"));

            var activeRect = rect;
            activeRect.width -= 20;
            activeRect.x += 20;
            EditorGUI.LabelField(new Rect(activeRect.width+15, activeRect.y, 45, activeRect.height), new GUIContent("Prior","Priority:\n If A mode has 'Ignore Lower Modes' enabled, it will play even if a Lower Mode is Playing"));
        }
        private void Draw_Element_Modes(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            EditorGUI.BeginChangeCheck();
            {
                ...
                var IDRect = new Rect(rect.x + 40, rect.y, rect.width - 60, EditorGUIUtility.singleLineHeight);
                ...
                EditorGUI.LabelField(new Rect(activeRect.width + 45, activeRect.y-1, 25, activeRect.height), "(" + (S_Mode_List.arraySize - index - 1) + ")");
